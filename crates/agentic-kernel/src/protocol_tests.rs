...
use super::{
    handle_hello, response_err_code, response_ok_code, stable_capabilities, CommandHeader, OpCode,
};
...
fn parse_basic_opcodes() {
    ...
    let stop_output = CommandHeader::parse("STOP_OUTPUT 1 9").expect("STOP_OUTPUT should parse");
...
fn parse_requires_three_tokens() {
    ...
    let err = CommandHeader::parse("PING 1 0 extra").expect_err("header with extra fields fails");
    assert!(err.to_string().contains("Invalid header format"));
}
