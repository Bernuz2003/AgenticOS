  ...
  return {
    ...
    replay: snapshot.replay
      ? {
          ...
          sourceSessionId: snapshot.replay.source_session_id,
          sourcePid: snapshot.replay.source_pid,
          sourceFidelity: snapshot.replay.source_fidelity,
          replayMode: snapshot.replay.replay_mode,
          toolMode: snapshot.replay.tool_mode,
          initialState: snapshot.replay.initial_state,
          patchedContextSegments: snapshot.replay.patched_context_segments,
          patchedEpisodicSegments: snapshot.replay.patched_episodic_segments,
          stubbedInvocations: snapshot.replay.stubbed_invocations,
          overriddenInvocations: snapshot.replay.overridden_invocations,
          baseline: {
            sourceContextSegments: snapshot.replay.baseline.source_context_segments,
            sourceEpisodicSegments: snapshot.replay.baseline.source_episodic_segments,
            sourceReplayMessages: snapshot.replay.baseline.source_replay_messages,
            sourceToolInvocations: snapshot.replay.baseline.source_tool_invocations,
            sourceContextChars: snapshot.replay.baseline.source_context_chars,
            sourceEpisodicChars: snapshot.replay.baseline.source_episodic_chars,
            sourceContextKinds: snapshot.replay.baseline.source_context_kinds,
            sourceEpisodicKinds: snapshot.replay.baseline.source_episodic_kinds,
          },
          diff: {
            currentContextSegments: snapshot.replay.diff.current_context_segments,
            currentEpisodicSegments: snapshot.replay.diff.current_episodic_segments,
            currentReplayMessages: snapshot.replay.diff.current_replay_messages,
            currentToolInvocations: snapshot.replay.diff.current_tool_invocations,
            contextChanged: snapshot.replay.diff.context_changed,
            contextSegmentsDelta: snapshot.replay.diff.context_segments_delta,
            episodicSegmentsDelta: snapshot.replay.diff.episodic_segments_delta,
            replayMessagesDelta: snapshot.replay.diff.replay_messages_delta,
            toolInvocationsDelta: snapshot.replay.diff.tool_invocations_delta,
            branchOnlyMessages: snapshot.replay.diff.branch_only_messages,
            branchOnlyToolCalls: snapshot.replay.diff.branch_only_tool_calls,
            changedToolOutputs: snapshot.replay.diff.changed_tool_outputs,
            completedToolCalls: snapshot.replay.diff.completed_tool_calls,
            latestBranchMessage: snapshot.replay.diff.latest_branch_message,
            invocationDiffs: snapshot.replay.diff.invocation_diffs.map((entry) => ({
              sourceToolCallId: entry.source_tool_call_id,
              replayToolCallId: entry.replay_tool_call_id,
              toolName: entry.tool_name,
              commandText: entry.command_text,
              sourceStatus: entry.source_status,
              replayStatus: entry.replay_status,
              sourceOutputText: entry.source_output_text,
              replayOutputText: entry.replay_output_text,
              branchOnly: entry.branch_only,
